...
export const checkRegistraionConfirmations = ({ firstRun }) => async (
  dispatch,
  getState
) => {
  if (firstRun) {
  ...
  setTimeout(async () => {
    ...
    const registrationTransaction = outgoingTransactions.filter(
      el => el.txid === txid
    )[0]
...
export const fetchUsers = (address, messages) => async (dispatch, getState) => {
  try {
    ...
    const { status: registrationStatus } = identitySelector.registrationStatus(
      getState()
    )
...
export const reducer = handleActions(
  {
    ...
    [mockOwnUser]: (state, { payload: { sigPubKey, nickname, address } }) =>
      produce(state, draft => {
        draft[sigPubKey] = {
          publicKey: sigPubKey,
          nickname,
